...
| Build | `npm ci` | `npm ci` |
| Test | `npm run build` 編譯檢查 | 獨立 `test` Job 執行編譯 |
| Package | Next.js 輸出 `out/` 靜態檔 | `docker build` → Docker Image |
| Deploy | 推到 `gh-pages` branch | Push 到 Docker Hub → Railway 拉 image |
